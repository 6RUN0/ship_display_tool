...
class fitting
{
	...
	function displayFitting() {
		...
		$fitting_array[27] = array();    // high slots
		$fitting_array[19] = array();    // med slots
		$fitting_array[11] = array();    // low slots
		$fitting_array[92] = array();    // rig slots
		$fitting_array[6] = array();    // drone bay
		$fitting_array[125] = array();    // subsystems
		...
		while($row = $qry->getRow()) {
			...
			if($row['itt_slot'] == 0) {
				if($row['itd_itl_id'] == 87) {
					...
					if($row['itd_itl_id'] == 19 || $row['itd_itl_id'] == 0) {
						$qry2 = new DBQuery();
				...
				$isFitted = true;
				if($row['itt_slot'] == 27 //high slot
					|| $row['itt_slot'] == 19 // med slot
					|| $row['itt_slot'] == 11 // low slot
					|| $row['itt_slot'] == 92 // rig slot
					|| $row['itt_slot'] == 125 // subsystem
				) {
					switch($row['itt_slot']){
						case 125:
							if($row['itd_itl_id'] < 125 || $row['itd_itl_id'] > 130){
								$isFitted = false;
							}
							break;
						case 92:
							if($row['itd_itl_id'] < 92 || $row['itd_itl_id'] > 94){
								$isFitted = false;
							}
							break;
						default:
							if($row['itd_itl_id'] < $row['itt_slot'] || $row['itd_itl_id'] > ($row['itt_slot'] + 7)){
								$isFitted = false;
							}
							break;
					}
				}
				if($row['itd_itl_id'] != 5 & $isFitted ) {
					for($i = 0; $i < $row['itd_quantity']; $i++) {
		...
		$temp = array();
		if(is_array($fitting_array[27]))
		{
			$hiammo = array();
			foreach ($fitting_array[27] as $highfit)
			{
				...
				if($group == 483 // Modulated Deep Core Miner II, Modulated Strip Miner II and Modulated Deep Core Strip Miner II
					...
					|| ($group >= 506 && $group <= 511) // Some Missile Launchers
					|| $group == 481 // Probe Launchers
					|| $group == 899 // Warp Disruption Field Generator I
					|| $group == 771 // Heavy Assault Missile Launchers
					|| $group == 589 // Interdiction Sphere Launchers
					|| $group == 524 // Citadel Torpedo Launchers
				){
					$found = 0;
					if ($group == 511){ $group = 509; } // Assault Missile Launchers uses same ammo as Standard Missile Launchers
		...
		if(is_array($fitting_array[19]))
		{
			$midammo = array();
			foreach ($fitting_array[19] as $midfit)
			{
